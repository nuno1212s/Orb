 ...
 */
public class SetNextPageCommand implements Command {

    @Override
    public String[] names() {
        return new String[]{"setnextpage"};
    }

    @Override
    public String usage() {
        return ChatColor.RED + "/market setnextpage <slots to last slot>";
    }

    @Override
    public void execute(Player player, String[] args) {
        if (!player.hasPermission("market.setnextpage")) {
        ...
        Main.getIns().getInventoryManager().setNextPageItem(slot, item);
        player.sendMessage(ChatColor.GREEN + "Next page item has been set.");
